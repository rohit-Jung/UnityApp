...
namespace NWH.WheelController3D
{
    ...
    public class StandardFrictionModel : IFrictionModel
    {
        ...
        public void SlipCircle(ref float Sx, ref float Sy, ref float Fx, ref float Fy)
        {
            if (Sx > 0.02f || Sx < -0.02f || Sy > 0.02f || Sy < -0.02f)
            {
                float SxClamped = Sx < -1f ? -1f : Sx > 1f ? 1f : Sx;
                float SyClamped = Sy < -1f ? -1f : Sy > 1f ? 1f : Sy;
                _combinedSlip.x = SxClamped;
                _combinedSlip.y = SyClamped;
                _slipDir = _combinedSlip.normalized;

                float absSlipDirY = _slipDir.y < 0 ? -_slipDir.y : _slipDir.y;
                Fy = Mathf.Abs(Fy) * absSlipDirY * (Fy < 0 ? -1f : 1f);

                float absSlipDirX = _slipDir.x < 0 ? -_slipDir.x : _slipDir.x;
                Fx = Mathf.Abs(Fx) * absSlipDirX * (Fx < 0 ? -1f : 1f);
            }
        }


        public void StepLongitudinal(float Tm, float Tb, float Vx, float Vy, ref float W, float L, float dt, float R,
            float I,
            AnimationCurve frictionCurve, float BCDEz, float kFx, float kSx, ref float Sx, ref float Fx, ref float Tcnt)
        {
            ...
            float WAbs  = W < 0 ? -W : W;
            float Lc = 14000f * (1f - Mathf.Exp(-0.00014f * L));
        ...
        public void StepLateral(float Vx, float Vy, float L, float dt, AnimationCurve frictionCurve, float BCDEz,
            float                     kFy,
            float                     kSy, ref float Sy, ref float Fy)
        {
            ...
            float VxAbs = Vx < 0 ? -Vx : Vx;
            float Lc = 22000f * (1f - Mathf.Exp(-0.0001f * L));
