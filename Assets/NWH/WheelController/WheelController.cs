...
namespace NWH.WheelController3D
{
    ...
    public class WheelController : MonoBehaviour
    {
        ...
        private float _rayDistanceThreshold;
        ...
        public void OnPrePhysicsSubstep(float t, float dt)
        {
            ...
            _localToWorldMatrix = _cachedTransform.localToWorldMatrix;
            _worldToLocalMatrix = _cachedTransform.worldToLocalMatrix;
            _transformPosition  = _cachedTransform.position;
            _transformRotation  = _cachedTransform.rotation;
            _transformForward   = _cachedTransform.forward;
            _transformRight     = _cachedTransform.right;
            _transformUp        = _cachedTransform.up;
        ...
        public void OnPhysicsSubstep(float t, float dt, int i)
        {
            ...
            frictionModel.StepLongitudinal(wheel.motorTorque, wheel.brakeTorque + dragTorque, forwardFriction.speed,
                                           sideFriction.speed,
                                           ref wheel.angularVelocity, wheel.load, _fixedDeltaTime,
            ...
            // Add squat torque
            _chassisTorque += forwardFriction.force * wheel.radius;
        ...
        public void OnPostPhysicsSubstep(float t, float dt)
        {
            ...
            parentNRigidbody.AddTorque(-_chassisTorque * wheel.right);
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (Application.isPlaying)
            {
                ...
                if (hasHit)
                {
                    ...
                    if (forwardFriction.speed < 2f && forwardFriction.speed > -2f)
                    {
                        Gizmos.color = Color.yellow;
                        Vector3 alternateNormal = (wheel.worldPosition - wheelHit.point).normalized;
                        Gizmos.DrawLine(wheelHit.point, wheelHit.point + alternateNormal * 0.3f);
                    }
        ...
        public float GetLongitudinalLoadCoefficient(float load)
        {
        ...
        /// </summary>
        /// <returns>Maximum friction force in N.</returns>
        public float GetLateralLoadCoefficient(float load)
        {
        ...
        /// </summary>
        // TODO - this is getting called mutliple times on start.
        public void InitializeScanParams()
        {
            ...
            {
                Debug.LogWarning("LongitudinalScanResolution must be > 0. Setting to 1.");
                longitudinalScanResolution = 1;
            }

            if (longitudinalScanResolution == 2)
            {
                Debug.LogWarning("LongitudinalScanResolution can not be 2. Select either 1 or >= 3. Setting to 3.");
                longitudinalScanResolution = 3;
            }

            if (lateralScanResolution < 1)
            {
                Debug.LogWarning("Side to side scan axisResolution must be > 0. Setting to 1.");
                lateralScanResolution = 1;
            }

            // Setup cached values
            layerMaskInt = (int)layerMask;

            _rayDistanceThreshold = spring.maxLength - spring.length;
            _rayLength = wheel.radius * 1.02f + _rayDistanceThreshold;
            ...
            // Increment
            _stepX = lateralScanResolution == 1 ? 1f : wheel.width / (lateralScanResolution - 1);
            _stepY = longitudinalScanResolution == 1 ? 1f : wheel.radius * 2f / (longitudinalScanResolution - 1);

            // Initialize wheel rays
            int totalRays = longitudinalScanResolution * lateralScanResolution;
            wheelHits = new WheelHit[totalRays];
            ...
            for (float x = _boundsX; x <= _boundsZ; x += _stepX)
            {
                ...
                for (float y = _boundsY; y <= _boundsW; y += _stepY)
                {
                    ...
                    if (longitudinalScanResolution == 1)
                    {
                        ...
                        wr.angleForward = 0;
                        wr.curvatureOffset = wheel.radius;
                        float xOffset = lateralScanResolution == 1 ? 0 : x;
                        wr.offset = new Vector2(xOffset, 0);
                        wheelHits[index] = wr;
            ...
            _raycastCommandsArray = new RaycastCommand[totalRays];

            _raycastHits = new NativeArray<RaycastHit>(totalRays, Allocator.Persistent);
            _raycastHitsArray = new RaycastHit[totalRays];

            // Pre-set data that will not change.
            for (int i = 0; i < totalRays; i++)
            {
                _raycastCommandsArray[i].distance = _rayLength + wheelHits[i].curvatureOffset;
                _raycastCommandsArray[i].layerMask = layerMaskInt;
        ...
        private void HitUpdate()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                Vector2 offset = wheelHits[i].offset;
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (_raycastHitsArray[i].distance > 0)
                {
                    ...
                    if (distanceFromTire > _rayDistanceThreshold)
                    {
                        continue;
                    }
        ...
        public void VisualUpdate()
        {
            ...
            _axleRotation       = Quaternion.AngleAxis(wheel.rotationAngle, _transformRight);
            wheel.worldRotation = steerQuaternion * _axleRotation * _transformRotation * Quaternion.Euler(wheel.visualRotationOffset);
